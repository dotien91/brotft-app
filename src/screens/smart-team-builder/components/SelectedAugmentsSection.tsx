...
import {useTftAugmentByApiName} from '@services/api/hooks/listQueryHooks';
...
const SelectedAugment = ({
  ...
  const {colors} = useTheme();
  const {data: augment} = useTftAugmentByApiName(apiName);

  return (
    <RNBounceable
      onPress={() => onRemove(apiName)}
      style={{overflow: 'visible'}}>
      <View style={styles.itemWrapper}>
        <View style={styles.itemBox}>
          <View pointerEvents="none" style={styles.innerItem}>
            <GuideAugmentItem
              data={augment || ({apiName} as any)}
              onPress={() => {}}
              isCompact={true}
            />
          </View>
